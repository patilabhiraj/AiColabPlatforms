...
void main() async {
  ...
  try {
    ...
    result.fold((_) => initialRoute = AppRouter.splash, (user) {
      if (user != null) {
        print(
          'DEBUG main(): Active session found for ${user.email}. Setting initial route directly to Chat!',
        );
        initialRoute = AppRouter.chat;
      } else {
        print(
          'DEBUG main(): No cached session. Setting initial route to Splash Page.',
        );
      }
    });
  } catch (e) {
